...
**Fase activa: F3 — Worker base Python (scaffold)**
...
| F3 — Worker base Python (scaffold) | 🔄 Siguiente | — |
| F4 — worker-whisper | ⏳ Pendiente | — |
...
- **API Go** en el VPS: todos los endpoints de F1 + ingesta de métricas (F2) + retención (F2)
...
Ver `docs/BACKLOG/f3.md` cuando se cree. Objetivos de la fase:
- Paquete Python `worker_base`: registro, heartbeat en thread separado, claim loop, fencing (409), helpers de progreso/logs/archivos
- Dockerfile base + `worker-echo` de ejemplo
- Prueba de integración real VPS ↔ ialab con workers en Docker
