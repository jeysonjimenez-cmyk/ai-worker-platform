...
- [ ] `git log` muestra el commit inicial con los 6 directorios.
- [ ] `cd api && go build ./...` termina sin error (aunque no compile nada útil).
- [ ] `cd workers && uv sync` termina sin error.
- [ ] `.gitignore` excluye `.env` y ningún secreto está versionado.
...
**Criterios de aceptación:**
- [ ] `make build` compila `api/` y valida sintaxis de `workers/`.
- [ ] `make help` lista todos los targets con una línea de descripción.
- [ ] Los placeholders existen y están documentados como pendientes.
...
**Criterios de aceptación:**
- [ ] `tailscale status` en el VPS muestra el nodo conectado como `vps`.
- [ ] Tras `reboot` del VPS, Tailscale reconecta solo (verificado).
...
**Criterios de aceptación:**
- [ ] `tailscale status` en ialab muestra el nodo conectado como `ialab`.
- [ ] Tras reinicio de ialab, reconecta solo (verificado).
- [ ] No se abrió ningún puerto de internet hacia ialab (revisar router/firewall).
...
**Criterios de aceptación:**
- [ ] Desde el VPS: `curl` a un puerto de prueba en ialab responde.
- [ ] Desde cualquier otro dispositivo del tailnet (ej. laptop personal): el mismo `curl` es rechazado.
- [ ] La ACL está copiada y explicada en `deploy/tailscale-acl.md`.
...
**Criterios de aceptación:**
- [ ] `ping` y `curl` del VPS a `ialab` (nombre Tailscale) funcionan.
- [ ] `nmap`/`curl` desde el VPS a la IP pública del hogar no expone ningún servicio de ialab.
- [ ] Resultados anotados en `deploy/tailscale-acl.md` con fecha.
...
**Criterios de aceptación:**
- [ ] `docker compose up -d` en el VPS deja PostgreSQL `healthy`.
- [ ] `psql` conecta desde el VPS (localhost); desde internet el puerto 5432 está cerrado.
- [ ] `docker compose restart` no pierde datos (tabla de prueba sobrevive).
- [ ] `curl localhost:8080/healthz` responde 200 desde el placeholder.
...
**Criterios de aceptación:**
- [ ] El cron genera un dump diario con timestamp en el nombre.
- [ ] Dumps con más de 7 días se eliminan automáticamente.
...
**Criterios de aceptación:**
- [ ] `make migrate-up` aplica la migración de prueba contra el PostgreSQL del VPS.
- [ ] `make migrate-down` la revierte limpiamente.
- [ ] El estado de migraciones es consultable (tabla de versiones).
...
**Criterios de aceptación:**
- [ ] `make deploy` desde la máquina de desarrollo deja PostgreSQL + placeholder corriendo en el VPS (criterio de la fase).
- [ ] Ejecutarlo dos veces seguidas es idempotente (segunda corrida sin cambios ni errores).
- [ ] Un cambio en el compose se refleja en el VPS tras `make deploy`.
...
**Criterios de aceptación:**
- [ ] El script ejecuta y muestra la RTX 4070 Ti SUPER con su VRAM total.
- [ ] Sale con código 0 en éxito y ≠0 si la GPU no es visible.
...
**Criterios de aceptación:**
- [ ] `make verify-infra` ejecuta todas las verificaciones y reporta ✓/✗ por ítem.
- [ ] Falla con código ≠0 si cualquier verificación falla (probado apagando un componente).
...
**Criterios de aceptación:**
- [ ] Un push con código Go que no compila pone la CI en rojo.
- [ ] Un push con error de lint Python pone la CI en rojo.
- [ ] Un push limpio pone la CI en verde en <5 min.
...
**Criterios de aceptación:**
- [ ] Siguiendo solo el README desde una máquina limpia se llega a `make verify-infra` en verde.
- [ ] La restauración de backup está documentada paso a paso.
...
| T0.6 | Validación de red | Seguridad | 2 | T0.5 |
| T0.7 | Compose VPS (PostgreSQL) | Infraestructura | 3 | T0.1 |
| T0.8 | Backups | DevOps | 3 | T0.7 |
...
**Paralelizables:** T0.3/T0.4/T0.11 no dependen del repo; pueden hacerse mientras avanza T0.1–T0.2.

**Cierre de fase:** la Fase 0 termina cuando `make verify-infra` está en verde y los 4 criterios de aceptación de la fase (deploy, conectividad Tailscale, no exposición de ialab, migración up/down) quedan demostrados — todos están cubiertos por T0.6, T0.9, T0.10 y T0.12.
